...
import torch
import numpy as np
from dynamics_net.utils.torch_utils import quat_to_tan_norm
import dynamics_net.env.tasks.humanoid_amp_task as humanoid_amp_task
from dynamics_net.env.tasks.humanoid_amp import HumanoidAMP, remove_base_rot
...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self._traj_skip = cfg["env"].get("trajSkip", 1)
        ...
        self._vert_dist_termination = cfg["env"].get("VerticalDistanceTermination", False)
    ...
    def _load_motion(self, motion_train_file, motion_test_file=[], subjects=[], failed_keys=False):
    ...
    def _load_marker_asset(self):
        asset_root = "dynamics_net/data/assets/urdf/"
    ...
    def post_physics_step(self):
        ...
        if flags.im_eval:
            ...
            res_pos = motion_res['rg_pos']
            res_rot = motion_res['rb_rot']
            self.extras['mpjpe'] = (body_pos - res_pos).norm(dim=-1).mean(dim=-1)
            self.extras['body_pos'] = body_pos.cpu().numpy()
            self.extras['body_pos_gt'] = res_pos.cpu().numpy()
            self.extras['body_rot'] = body_rot.cpu().numpy()
            self.extras['body_rot_gt'] = res_rot.cpu().numpy()

        return

    def _compute_observations(self, env_ids=None, reset=False):
        ...
        if (self._enable_task_obs):
            task_obs = self._compute_task_obs(env_ids, reset=reset)
    ...
    def _compute_task_obs(self, env_ids=None, save_buffer = True, reset=False):
        if (env_ids is None):
            body_pos = self._rigid_body_pos
            body_rot = self._rigid_body_rot
    ...
    def _compute_reset(self):
        ...
        if self.cycle_motion:
            ...
            if pass_time_motion_len.sum() > 0:
                ...
                self._cycle_counter[pass_time_motion_len] = 100
        ...
        if self.zero_out_far and self.zero_out_far_train:
            ...
            if self._vert_dist_termination:
                self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_im_reset_vertical(self.reset_buf, self.progress_buf, self._contact_forces, self._contact_body_ids, \
        ...
        # # Debug: inspect motion keys for sequences flagged as terminated
        # if self._terminate_buf.sum() > 0:
        #     terminated_env_ids = self._terminate_buf.nonzero(as_tuple=True)[0]
        #     print(f"Terminated environments: {terminated_env_ids}")

        #     # Inspect current motion keys
        #     if hasattr(self, '_motion_lib') and hasattr(self._motion_lib, '_curr_motion_ids'):
        #         curr_motion_ids = self._motion_lib._curr_motion_ids
        #         curr_motion_keys = self._motion_lib._motion_data_keys[curr_motion_ids]

        #         print("Current motion keys:")
        #         for env_id in terminated_env_ids:
        #             env_id = env_id.item()
        #             motion_id = curr_motion_ids[env_id].item()
        #             motion_key = curr_motion_keys[env_id]
        #             print(f"  Environment {env_id}: Motion ID {motion_id}, Key: {motion_key}")

        #         # Extract only the failed motion keys
        #         failed_motion_keys = curr_motion_keys[terminated_env_ids]
        #         print(f"Failed motion keys: {failed_motion_keys}")
...
def compute_imitation_observations_v6(root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, time_steps, upright):
    ...
    diff_local_body_pos_flat = torch_utils.my_quat_rotate(heading_inv_rot_expand.reshape(-1, 4), diff_global_body_pos.reshape(-1, 3))
    ...
    diff_local_vel = torch_utils.my_quat_rotate(heading_inv_rot_expand.reshape(-1, 4), diff_global_vel.reshape(-1, 3))

    diff_global_ang_vel = ref_body_ang_vel.reshape(B, time_steps, J, 3) - body_ang_vel.reshape(B, 1, J, 3)
    diff_local_ang_vel = torch_utils.my_quat_rotate(heading_inv_rot_expand.reshape(-1, 4), diff_global_ang_vel.reshape(-1, 3))
    ...
    local_ref_body_pos = torch_utils.my_quat_rotate(heading_inv_rot_expand.reshape(-1, 4), local_ref_body_pos.reshape(-1, 3))
    ...
    obs.append(diff_local_body_pos_flat.reshape(B, time_steps, -1))  # 1 * timestep * 24 * 3
    obs.append(diff_local_body_rot_flat.reshape(B, time_steps, -1))  #  1 * timestep * 24 * 6
    obs.append(diff_local_vel.reshape(B, time_steps, -1))  # timestep  * 24 * 3
    obs.append(diff_local_ang_vel.reshape(B, time_steps, -1))  # timestep  * 24 * 3
    obs.append(local_ref_body_pos.reshape(B, time_steps, -1))  # timestep  * 24 * 3
    obs.append(local_ref_body_rot.reshape(B, time_steps, -1))  # timestep  * 24 * 6

    # RH: add global info
    # obs.append(diff_global_body_pos.reshape(B, time_steps, -1))  # 1 * timestep * 24 * 3
    # obs.append(torch_utils.quat_to_tan_norm(diff_global_body_rot).reshape(B, time_steps, -1))  #  1 * timestep * 24 * 6
...
@torch.jit.script
def compute_humanoid_im_reset_vertical(reset_buf, progress_buf, contact_buf, contact_body_ids, rigid_body_pos, ref_body_pos, pass_time, enable_early_termination, termination_distance, disableCollision, use_mean):
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, bool, Tensor, bool, bool) -> Tuple[Tensor, Tensor]
    terminated = torch.zeros_like(reset_buf)
    if (enable_early_termination):
        has_fallen = torch.any(torch.abs(rigid_body_pos[..., 2] - ref_body_pos[..., 2]) > termination_distance, dim=-1)  # using max
        # first timestep can sometimes still have nonzero contact forces
        # so only check after first couple of steps
        has_fallen *= (progress_buf > 1)
        if disableCollision:
        ...
        # if terminated.sum() > 0:
        #     # The following can be inspected at this point:
        #     # - terminated: termination flag
        #     # - has_fallen: failure judgement
        #     # - rigid_body_pos: current body positions
        #     # - ref_body_pos: reference body positions
        #     # - termination_distance: termination distance

        #     # Inspect indices of failed envs
        #     failed_env_ids = terminated.nonzero(as_tuple=True)[0]
        #     print(f"Failed environment IDs: {failed_env_ids}")

        #     # Detailed info per failed env
        #     for env_id in failed_env_ids:
        #         env_id = env_id.item()
        #         print(f"Environment {env_id}:")
        #         print(f"  - Progress: {progress_buf[env_id]}")
        #         print(f"  - Has fallen: {has_fallen[env_id]}")
        #         print(f"  - Termination distance: {termination_distance}")

        #         # Inspect position differences
        #         pos_diff = torch.abs(rigid_body_pos[env_id, ..., 2] - ref_body_pos[env_id, ..., 2])
        #         max_diff = pos_diff.max()
        #         print(f"  - rigid_body_pos: {rigid_body_pos[env_id, ..., 2]}")
        #         print(f"  - ref_body_pos: {ref_body_pos[env_id, ..., 2]}")
        #         print(f"  - pos_diff: {pos_diff}")
        #         print(f"  - Max vertical position difference: {max_diff:.4f}")
        #         print(f"  - Position difference exceeds threshold: {max_diff > termination_distance}")
