...
class MotionLibBase():
    ...
    def get_pkl_files(self, motion_file, subjects=[]):
        if not subjects:
            subject_ids = range(1, 7)  # 1〜6
        else:
            subject_ids = subjects

        pkl_files = []
        for sid in subject_ids:
            subject_dir = os.path.join(motion_file, f"subj{sid:03d}")
            pkl_files.extend(glob.glob(os.path.join(subject_dir, "*.pkl")))

        return pkl_files

    def load_data(self, motion_file, min_length=-1, im_eval=False, subjects=None):
        if osp.isfile(motion_file):
            self.mode = MotionlibMode.file
            self._motion_data_load = joblib.load(motion_file)
            ...
            if min_length != -1:
                data_list = {
                    k: v for k, v in self._motion_data_load.items()
                    if len(v['pose_quat_global']) >= min_length
                }
            elif im_eval:
                data_list = {
                    item[0]: item[1]
                    for item in sorted(self._motion_data_load.items())
                    # for item in sorted(
            ...
            # pkl_files = self.get_pkl_files(motion_file, subjects=subjects)

            # data_list = {}
            # for pkl_file in pkl_files:
            #     loaded_data = joblib.load(pkl_file) 
            #     key = pkl_file.split('/')[-1].split('.')[0]
            #     data_list[key] = loaded_data
            ...
            for pkl_file in pkl_files:
                loaded_data = joblib.load(pkl_file) 
                merged_dict.update(loaded_data)
            ...
            if min_length != -1:
                ...
                data_list = {
                    item[0]: item[1]
                    for item in sorted(merged_dict.items())
                    # for item in sorted(
                    #     merged_dict.items(),
                    #     key=lambda entry: len(entry[1]['pose_quat_global']),
                    #     reverse=False
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        #     res = self.load_motion_with_skeleton(*jobs[i], None, i)
        #     res_acc.update(res)
