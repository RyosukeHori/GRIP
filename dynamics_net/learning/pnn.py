...
from dynamics_net.learning.network_builder import NetworkBuilder
...
class PNN(NetworkBuilder.BaseNetwork):
    ...
    def set_primitives(self, model_path, training_prim=1):
        checkpoint = torch_ext.load_checkpoint(model_path)
        state_dict_load = checkpoint['model']

        # Load the weights of the previous primitives
        for idx in range(training_prim):
            state_dict = self.actors[idx].state_dict()
            for k in state_dict_load.keys():
                if f"pnn.actors.{idx}" in k:
                    pnn_dict_key = k.split(f"pnn.actors.{idx}.")[1]
                    state_dict[pnn_dict_key].copy_(state_dict_load[k])

        # Copy the weights of the current primitive
        state_dict = self.actors[training_prim].state_dict()
        for k in state_dict_load.keys():
            if f"pnn.actors.{training_prim}" in k:
                pnn_dict_key = k.split(f"pnn.actors.{training_prim}.")[1]
                state_dict[pnn_dict_key].copy_(state_dict_load[k])

        # Freeze the weights of the previous primitives
        for param in self.actors[:training_prim].parameters():
            param.requires_grad = False
        if self.has_lateral:
            for param in self.u[:training_prim - 1].parameters():
                param.requires_grad = False

        # Update the running mean and variance
        self.running_mean, self.running_var = checkpoint['running_mean_std']['running_mean'], checkpoint['running_mean_std']['running_var']
