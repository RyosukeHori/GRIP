...
from dynamics_net.learning.amp_network_builder import AMPBuilder
import torch
import torch.nn as nn
import numpy as np
import copy
from dynamics_net.learning.pnn import PNN
from rl_games.algos_torch import torch_ext
...
class AMPPNNBuilder(AMPBuilder):
    ...
    class Network(AMPBuilder.Network):
        ...
        def __init__(self, params, **kwargs):
            ...
            self.model_base = self.task_obs_size_detail['models_path']
            ...
            # Load and copy weights from previous primitives
            if self.training_prim > 0:
                assert len(self.model_base) == 1
                self.pnn.set_primitives(self.model_base[0], self.training_prim)
            else:
