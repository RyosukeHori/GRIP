...
from dynamics_net.env.tasks.humanoid_amp_task import HumanoidAMPTask

import dynamics_net.learning.replay_buffer as replay_buffer
import dynamics_net.learning.common_agent as common_agent

from tensorboardX import SummaryWriter
import copy
from dynamics_net.utils.torch_utils import project_to_norm
import dynamics_net.learning.amp_datasets as amp_datasets
from dynamics_net.learning.loss_functions import kl_multi
...
class AMPAgent(common_agent.CommonAgent):
    ...
    def __init__(self, base_name, config):
        ...
        self.save_grf_info = self.vec_env.env.task.cfg.env.get("save_grf_info", False)
        self.only_kin_loss = self.vec_env.env.task.cfg.env.get("only_kin_loss", False)
        self.kin_grf_loss = self.vec_env.env.task.cfg.env.get("kin_grf_loss", False)
        ...
        if self.save_grf_info:
            self.grf_dict_info = None
    ...
    def set_full_state_weights(self, weights):
        ...
        if "kin_optimizer" in weights and "kin_optimizer" in self.__dict__:
    ...
    def init_tensors(self):
        ...
        if self.save_grf_info:
            B, S, _ = self.experience_buffer.tensor_dict['obses'].shape
            grf_dict = self.vec_env.env.task.grf_dict
            grf_dict_size = np.sum([v.reshape(v.shape[0], -1).shape[-1] for k, v in grf_dict.items()])
            self.experience_buffer.tensor_dict['grf_dict'] = torch.zeros((B, S, grf_dict_size)).to(self.experience_buffer.tensor_dict['obses'])
            self.tensor_list += ['grf_dict']
    ...
    def play_steps_rnn(self):
        ...
        for n in range(self.horizon_length):
            ...
            if self.only_kin_loss or self.kin_grf_loss:
            ...
            if self.kin_grf_loss:
                self.experience_buffer.update_data_rnn('grf_dict', indices, play_mask, torch.cat([v.reshape(v.shape[0], -1) for k, v in infos['grf_dict'].items()], dim = -1))
                if self.grf_dict_info is None:
                    self.grf_dict_info = {k: (v.shape, v.reshape(v.shape[0], -1).shape) for k, v in infos['grf_dict'].items()}
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            if (self.only_kin_loss or self.kin_grf_loss) and self.save_kin_info:
                # pure behavior cloning, kinemaitc loss. 
                self.obs, rewards, self.dones, infos = self.env_step(res_dict['mus'])
            else:
                self.obs, rewards, self.dones, infos = self.env_step(res_dict['actions'])
            ...
            if self.save_grf_info:
                self.experience_buffer.update_data('grf_dict', n, torch.cat([v.reshape(v.shape[0], -1) for k, v in infos['grf_dict'].items()], dim = -1))
                if self.grf_dict_info is None:
                    self.grf_dict_info = {k: (v.shape, v.reshape(v.shape[0], -1).shape) for k, v in infos['grf_dict'].items()}
    ...
    def prepare_dataset(self, batch_dict):
        ...
        if self.save_grf_info:
            dataset_dict['grf_dict'] = batch_dict['grf_dict']
    ...
    def train_epoch(self):
        ...
        if self.save_kin_info:
            print_str = "Kin: " + " \t".join([f"{k}: {torch.mean(torch.tensor(train_info[k])):.4f}" for k, v in train_info.items() if k.startswith("kin") or k.startswith("action") or k.startswith("joint") or k.startswith("vel")])

            if self.save_grf_info:
                grf_str = "GRF: " + " \t".join([f"{k}: {torch.mean(torch.tensor(train_info[k])):.4f}" for k, v in train_info.items() if k.startswith("grf")])
                total_str = "Total: " + " \t".join([f"{k}: {torch.mean(torch.tensor(train_info[k])):.4f}" for k, v in train_info.items() if k.startswith("total")])
                print_str = f"{print_str} \t {grf_str} \t {total_str}"
            print(print_str)
    ...
    def calc_gradients(self, input_dict):
        ...
        if self.only_kin_loss:
        ...
        elif self.kin_grf_loss:
            batch_dict = {}
            batch_dict['obs_orig'] = obs_batch
            batch_dict['obs'] = input_dict['obs_processed']
            batch_dict['kin_dict'] = input_dict['kin_dict']
            batch_dict['grf_dict'] = input_dict['grf_dict']

            grf_loss_info = self._optimize_kin_grf(batch_dict)
            self.train_result.update( {'entropy': torch.tensor(0).float(), 'kl': torch.tensor(0).float(), 'last_lr': self.last_lr, 'lr_mul': torch.tensor(0).float()})
        else:
            ...
            self.train_result.update( {'entropy': entropy, 'kl': kl_dist, 'last_lr': self.last_lr, 'lr_mul': lr_mul, 'b_loss': b_loss, 'total_loss': loss})
        ...
        if self.save_kin_info:
            if self.kin_grf_loss:
                self.train_result.update(grf_loss_info)
            else:
                self.train_result.update(kin_loss_info)

        return

    def _assemble_kin_dict(self, kin_dict_flat):
    ...
    def _assemble_grf_dict(self, grf_dict_flat):
        B = grf_dict_flat.shape[0]
        len_acc = 0
        grf_dict = {}
        for k, v in self.grf_dict_info.items():
            grf_dict[k] = grf_dict_flat[:, len_acc:(len_acc + v[1][-1])].view(B, *v[0][1:])
            len_acc += v[1][-1]
        return grf_dict
    ...
    def _optimize_kin(self, batch_dict):
        ...
        if humanoid_env.distill: 
            kin_dict = self._assemble_kin_dict(batch_dict['kin_dict'])
            ...
            if humanoid_env.z_type == "vae":
                ...
                if humanoid_env.refiner:
                    pred_action, pred_action_sigma, extra_dict = self.model.a2c_network.eval_actor(batch_dict, return_extra=True)
                    action_loss = torch.norm(pred_action - gt_action, dim=-1).mean()  ## RMSE

                    vel_loss = torch.norm(extra_dict['pred_vel'] - kin_dict['gt_vel'], dim=-1).mean()  ## RMSE
                    vel_loss = vel_loss * humanoid_env.vel_loss_weight

                    pred_joints = extra_dict['pred_joints']  # (B, T, 24, 3)
                    gt_joints = kin_dict['gt_joints']  # (B, T, 24, 3)
                    _, T, J, _ = pred_joints.shape
                    trans_offset = gt_joints[:, 0, 0, :2] - pred_joints[:, 0, 0, :2]
                    pred_joints[..., :2] += trans_offset[:, None, None, :].repeat(1, T, J, 1)
                    joint_loss = torch.norm(pred_joints - gt_joints, dim=-1).mean()  ## RMSE
                    joint_loss = joint_loss * humanoid_env.joint_loss_weight
                    
                    info_dict.update({"action_loss": action_loss})
                    info_dict.update({"vel_loss": vel_loss})
                    info_dict.update({"joint_loss": joint_loss})

                    kin_loss = action_loss + joint_loss + vel_loss
    ...
    def _optimize_kin_grf(self, batch_dict):
        info_dict = {}
        humanoid_env = self.vec_env.env.task
        if humanoid_env.distill: 
            ## Kinematic Loss
            ...
            ## GRF Loss
            grf_dict = self._assemble_grf_dict(batch_dict['grf_dict'])
            sim_grf = grf_dict['sim_grf']
            sensor_grf = grf_dict['sensor_grf']
            grf_loss = torch.norm(sim_grf - sensor_grf, dim=-1).mean() * 0.001  ## RMSE

            total_loss = kin_loss + grf_loss
            ...
            info_dict.update({"kin_loss": kin_loss, "grf_loss": grf_loss, "loss": total_loss})
    ...
    def _assemble_train_info(self, train_info, frame):
        ...
        if "kin_loss" in train_info:
            train_info_dict['kin_loss'] = torch_ext.mean_list(train_info['kin_loss']).item()
            
        if "grf_loss" in train_info:
            train_info_dict['grf_loss'] = torch_ext.mean_list(train_info['grf_loss']).item()

        if "vel_loss" in train_info:
            train_info_dict['vel_loss'] = torch_ext.mean_list(train_info['vel_loss']).item()

        if "joint_loss" in train_info:
            train_info_dict['joint_loss'] = torch_ext.mean_list(train_info['joint_loss']).item()
        
        if "action_loss" in train_info:
            train_info_dict['action_loss'] = torch_ext.mean_list(train_info['action_loss']).item()

        if "total_loss" in train_info:
            train_info_dict['total_loss'] = torch_ext.mean_list(train_info['total_loss']).item()
