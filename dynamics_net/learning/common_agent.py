    ...
    def train(self):
        ...
        while True:
            ...
            if hasattr(self.vec_env, 'env') and hasattr(self.vec_env.env, 'task'):
                self.vec_env.env.task.current_epoch = epoch_num
            ...
            if self.rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if np.mean(mean_rewards) > best_reward and epoch_num > self.save_best_after:
                        self.save(model_output_file)
                        best_reward = np.mean(mean_rewards)
                        log_str += " (best)"
                    # if train_info_dict["returns"] > best_reward and epoch_num > self.save_best_after:
                    #     self.save(model_output_file)
                    #     best_reward = train_info_dict["returns"]
                    #     log_str += f" (best reward: {best_reward:.1f})"
                    print(log_str)
